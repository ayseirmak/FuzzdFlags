...
/* The global list of mutator names */
...
void log_diff_result(const char *original, const char *mutated) {
    char diffCmd[4096];
    snprintf(diffCmd, sizeof(diffCmd),
             "diff -q '%s' '%s' > /dev/null 2>&1", original, mutated);

    int rc = system(diffCmd);

    /* If rc == 0 => files are same, else => differ or error. */
    const char *diffOutcome = (rc == 0) ? "NO" : "YES";

    FILE *logf = fopen("mutation_diff.log", "a");
    if (!logf) {
        perror("[!] fopen mutation_diff.log");
        return;
    }
    fprintf(logf, "Original: %s, Mutated: %s => Different? %s\n",
            original, mutated, diffOutcome);
    fclose(logf);
}
