...
# Download input-seeds
RUN wget https://github.com/ayseirmak/FuzzdFlags/releases/download/v2.0-beta/exp22-fuzzdflags-input-seeds.tar.gz && \
    tar -zxvf exp22-fuzzdflags-input-seeds.tar.gz
...
# Set environment variables for instrumented clang and C files directory
ENV INSTRUMENTED_CLANG_PATH=/users/user42/build/bin/clang
ENV CFILES_DIR=/users/user42/llvmSS-reindex-cfiles/
ENV FILE_COUNT=1705
